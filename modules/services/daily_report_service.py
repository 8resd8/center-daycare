...
class EvaluationService:
    ...
    def save_special_note_evaluation(
        self, record_id: int, evaluation_result: dict
    ) -> None:
        """특이사항 평가 결과를 DB에 저장
    ...
    def _save_evaluation_to_db(
        ...
        existing = self.db_repo._execute_query_one(
            check_query, (record_id, korean_category)
        )
        ...
        if existing:
            ...
            """
            self.db_repo._execute_transaction(
                update_query,
                (
                    oer_fidelity,
                    specificity,
    ...
    def evaluate_special_note_with_ai(self, record: dict) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error("AI 클라이언트 초기화 오류: %s", e)
            return None
        ...
        try:
            ...
            logger.debug(
                "AI 평가 결과 — 신체: %s, 인지: %s",
                result.get("original_physical_evaluation", {}),
                result.get("original_cognitive_evaluation", {}),
            )
            ...
            # O/X 평가 결과를 점수로 변환
            result["original_physical"] = self._convert_ox_to_score(
                result["original_physical"]
            )
            result["original_cognitive"] = self._convert_ox_to_score(
                result["original_cognitive"]
            )
            ...
            logger.error("특이사항 AI 평가 중 오류 발생: %s", e)
            return None
    ...
    def _select_most_unique_sentences(
        self, result: Dict, previous_sentences: List[str]
    ) -> Dict:
        ...
        physical_sentences = [
            candidate["corrected_note"] for candidate in result["physical_candidates"]
        ]
        ...
        cognitive_sentences = [
            candidate["corrected_note"] for candidate in result["cognitive_candidates"]
        ]
    ...
    def _find_least_similar(
        self, candidates: List[str], references: List[str], vectorizer: Any
    ) -> str:
    ...
    def save_ai_evaluation(
        ...
        if evaluation_result:
            ...
            specificity_score = evaluation_result.get(
                "specificity_score"
            ) or evaluation_result.get("specificity", "X")
            # grammar와 grammar_score 모두 지원
            grammar_score = evaluation_result.get(
                "grammar_score"
            ) or evaluation_result.get("grammar", "X")
            grade_code = evaluation_result.get("grade_code", "평가없음")
            reason_text = evaluation_result.get(
                "reasoning_process", ""
            ) or evaluation_result.get("reason", "")
            suggestion_text = evaluation_result.get(
                "suggestion_text", ""
            ) or evaluation_result.get("corrected_note", "")
        else:
            oer_fidelity = "X"
            specificity_score = "X"
            grammar_score = "X"
    ...
    def process_daily_note_evaluation(
        self,
        record_id: int,
        category: str,
        note_text: str,
        ...
        if not note_text or note_text.strip() in ["특이사항 없음", "결석", ""]:
            # 빈 텍스트는 평가하지 않음
            return {"grade_code": "평가없음", "evaluation": None}
        ...
        if category in ["PHYSICAL", "COGNITIVE"]:
            ...
            if ai_result:
                ...
                if category == "PHYSICAL":
                    ...
                    corrected_note = ai_result.get("physical", {}).get(
                        "corrected_note", ""
                    )
                elif category == "COGNITIVE":
                    evaluation_result = ai_result.get("original_cognitive", {})
                    # 수정 제안(corrected_note)도 포함
                    corrected_note = ai_result.get("cognitive", {}).get(
                        "corrected_note", ""
                    )
        ...
        self.save_ai_evaluation(
            record_id, category, note_writer_user_id, evaluation_result, note_text
        )

        return {"grade_code": korean_grade, "evaluation": evaluation_result}
